	...
	cbGender
		.map(
			(item) =>
				...
				(student.gender === item.code
					? `<input type="radio" name="gender" class="form-check-input" value="${item.code}" checked /> ${item.names.en}`
					: `<input type="radio" name="gender" class="form-check-input" value="${item.code}" /> ${item.names.en}`) +
				`</label>`
		)
		.join("");
...
const getHouseSelectString = (student, cbHouse) =>
	...
	cbHouse
		.map((item) =>
			student.house === item.code
				? `<option value=${item.code} selected>${item.names.en}</option>`
				: `<option value=${item.code}>${item.names.en}</option>`
		)
		.join("") +
	`</select>`;
